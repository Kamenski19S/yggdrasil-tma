...
type Screen = { t: "tree" } | { t: "realm"; id: string } | { t: "choose" } | { t: "hero" } | { t: "gift" } | { t: "hall" };
type Save = { sparks: number; done: string[]; gift: string; hero: { id: string; name: string } | null };
const DEF: Save = { sparks: 25, done: [], gift: "", hero: null };
...
/* Экраны: выбор судьбы, герой, дар, чертог */
    ...
    <div className="app">
      ...
      {screen.t === "realm" && (() => {
        ...
        return (
          <div className="content">
            ...
            <button className="gate" onClick={() => say("Врата запечатаны. Хозяин мира ждёт героя в следующем обновлении.")}>
              ...
              <span className="mname" style={{ color: realm.color, borderColor: realm.glow }}>Врата мира</span>
            </button>
            <div className="hint">Врата откроются, когда придёт час испытаний</div>
          </div>
        );
      })()}
      ...
      {screen.t === "hero" && heroDef && save.hero && (
        <div className="scroll">
          <div className="card center">
            ...
            <div className="hrow">✨ Искр: <b>{save.sparks}</b> • 🏺 Артефакты: 0/9 (впереди)</div>
          </div>
        </div>
      )}
      ...
      {screen.t === "hall" && (
        <div className="scroll"><div className="card center"><div className="big">🏛️</div><div className="qhead2">Чертог путника</div>
          <div className="stats"><div className="stat"><b>✨ {save.sparks}</b><span>Искр</span></div><div className="stat"><b>🏺 0/9</b><span>артефакты</span></div></div>
          <div className="rank">🏆 Ранг: {rank(save.sparks)}</div>
          {save.hero && heroDef && <p className="dim">Герой: {save.hero.name} • {heroDef.race}</p>}
        </div></div>
      )}
